...
build-image: ${OE_ENV_FILE_PATH}
	docker run --volume ${THIS_MAKEFILE_DIR}:${THIS_MAKEFILE_DIR} \
            ...
            bash -c "source ${OE_ENV_FILE_PATH} && bitbake u-boot-ec900 -c cleansstate && bitbake u-boot-ec900 -c clean && bitbake -v -D u-boot-ec900 && bitbake ec900-image -c do_updateimg"
            # bash -c "source ${OE_ENV_FILE_PATH} && bitbake u-boot-ec900 -c cleansstate && bitbake u-boot-ec900 -c clean && bitbake u-boot-ec900 -c menuconfig"
            # bash -c "source ${OE_ENV_FILE_PATH} && bitbake u-boot-ec900 -c cleansstate && bitbake u-boot-ec900 -c clean && bitbake -v -D u-boot-ec900 && bitbake ec900-image -c do_updateimg"
